...
let AddressService = class AddressService {
    ...
    async findOne(address_id) {
        const address = await this.addressRepository.findOne({
            where: { address_id: address_id },
            relations: [],
        });
        if (!address) {
            throw new common_1.NotFoundException(`Address with ID ${address_id} not found`);
        }
        return address;
    }
    update(address_id, updateAddressDetails) {
        return this.addressRepository.update({ address_id }, { ...updateAddressDetails });
    }
    async remove(address_id) {
